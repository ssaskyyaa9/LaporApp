...
import { daftarPengguna } from "@/lib/api";
...
export default function HalamanDaftar() {
  ...
  const tanganiDaftar = async (e) => {
    ...
    setPesan("");
    try {
      await daftarPengguna({ username: namapengguna, email, password: kataSandi });
      router.push("/masuk?daftar=berhasil");
    } catch (err) {
    ...
    <main className="min-h-screen flex items-center justify-center" style={{ backgroundColor: "#DDE8F8" }}>
      ...
      <div className="bg-white rounded-3xl shadow-lg px-10 py-10 w-full max-w-md">
        ...
        <div className="text-center mb-7">
          ...
          <p className="text-sm text-gray-400">Daftar untuk membuat akun Anda</p>
        </div>

        <form onSubmit={tanganiDaftar} className="flex flex-col gap-4">
          <div className="flex items-center gap-3 border border-gray-200 rounded-xl px-4 py-3 focus-within:border-blue-400 transition-colors">
            <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
              <circle cx="9" cy="6" r="3.5" stroke="#9CA3AF" strokeWidth="1.5" />
              <path d="M2 16c0-3.314 3.134-6 7-6s7 2.686 7 6" stroke="#9CA3AF" strokeWidth="1.5" strokeLinecap="round" />
            </svg>

            <input type="text" placeholder="Nama Pengguna" value={namapengguna}
            ...
            <button type="button" onClick={() => setTampilkanSandi(!tampilkanSandi)}
              ...
              {tampilkanSandi ? (
                <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
                  ...
                  <path d="M3 3l12 12" stroke="#9CA3AF" strokeWidth="1.5" strokeLinecap="round" />
                </svg>
              ) : 
        ...
        <p className="text-center text-sm text-gray-500 mt-5">
          Sudah punya akun?{" "}
          <Link href="/masuk" className="text-blue-600 font-semibold hover:underline">Masuk sekarang</Link>
        </p>
      </div>
    </main>
  );
