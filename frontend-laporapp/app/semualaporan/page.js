...
import { ambilToken, ambilKategori, ambilSemuaLaporan } from "@/lib/api";
...
const warnaKategori = {
  "Infrastruktur":           "bg-orange-50 text-orange-500",
  "Lingkungan Hidup":        "bg-emerald-50 text-emerald-600",
  "Keamanan & Ketertiban":   "bg-blue-50 text-blue-600",
  "Sosial & Kemasyarakatan": "bg-red-50 text-red-500",
  "Lainnya":                 "bg-purple-50 text-purple-600",
};

export default function HalamanSemuaLaporan() {
  ...
  useEffect(() => {
    ...
    try {
      const [resLaporan, resKategori, resUnread] = await Promise.all([
        ambilSemuaLaporan(),
        ambilKategori(),
      ...
      const resProfil = await fetch(`${BASE_URL}/api/profile`, { 
        headers: { Authorization: `Bearer ${ambilToken()}` } 
      }).then(r => r.json());
      if (resProfil?.data) 
        setUser((prev) => ({ ...prev, foto: resProfil.data.foto, username: resProfil.data.username })
      );
      ...
      <main className="flex-1 transition-all duration-300 min-h-screen" style={{ marginLeft: lebarSidebar }}>
        <header className="bg-[#EDF3FD] border-b border-gray-100 px-6 py-3 flex items-center justify-between sticky top-0 z-30">
          <div className="flex items-center gap-2 text-xs text-gray-500">
            ...
            <span>›</span>
            <span className="text-blue-600 font-semibold">Semua Laporan</span>
          </div>
        ...
        <div className="p-6">
          <div className="mb-4">
            ...
            <p className="text-gray-500 text-sm mt-0.5">Laporan terbaru yang diajukan sesama pengguna LaporApp.</p>
          </div>
          ...
          {memuat ? (
            ...
            <div className="flex flex-col gap-3">
              {laporanTampil.map((item) => {
                ...
                const warnaKat = warnaKategori[item.nama_kategori] || "bg-orange-50 text-orange-500";
                ...
                return (
                  <div key={item.id_laporan} className="bg-white rounded-2xl shadow-sm border border-gray-100 flex items-stretch overflow-hidden">
                    ...
                    <div className="flex-1 py-4 pr-5 min-w-0 p-3">
                      <div className="flex items-center justify-between gap-2 mb-2">
                        <span className={`text-xs font-semibold px-2 py-0.5 rounded-md ${warnaKat}`}>
                      ...
                      <p className="font-bold text-gray-800 text-sm leading-snug mb-1">{item.judul}</p>
                      <p className="text-gray-500 text-xs leading-relaxed mb-2" style={{ display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden", maxWidth: "520px", wordBreak: "break-word", }}> {item.deskripsi} </p>
                      ...
                      <div className="flex items-center justify-between mt-1">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex items-center gap-1">
                            <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="2">
                              <circle cx="12" cy="12" r="10"/><path d="M12 11c1.66 0 3-1.34 3-3s-1.34-3-3-3-3 1.34-3 3 1.34 3 3 3z"/><path d="M6 20c0-3.31 2.69-6 6-6s6 2.69 6 6"/>
                            </svg>
                            <p className="text-gray-400 text-xs">Dibuat oleh <span className="font-semibold text-gray-600">{item.username}</span></p>
                          </div>
                        </div>
                        
                        <Link href={`/laporan/${item.id_laporan}`}
                          className="text-blue-600 text-xs font-semibold hover:underline shrink-0">
                          Lihat detail
                        </Link>
                      </div>
                    </div>
