...
export default function HalamanProfileAdmin() {
  ...
  // State statistik disesuaikan dengan dashboard admin
    ...
    const muat = async () => {
      ...
      try {
        const [resProfil, totalUnread, resStatistik] = await Promise.all([
          ambilProfil(),
          fetchUnread(),
          ambilStatistikAdmin(), // Mengambil data statistik dari API Admin
        ]);
  ...
  const simpan = async () => {
    ...
    try {
      ...
      setFotoFile(null);
      setFotoPreview(null);
      setPesan({ tipe: "sukses", teks: "Profil Admin berhasil diperbarui!" });
  ...
  // Konfigurasi visual badge berdasarkan role dinamis ("admin" atau "super admin")
  const roleConfig = {
    "admin": { label: "Admin", color: "text-amber-600", bg: "bg-amber-50", border: "border-amber-200", dot: "bg-amber-500" },
    "super admin": { label: "SuperAdmin", color: "text-emerald-600", bg: "bg-emerald-50", border: "border-emerald-200", dot: "bg-emerald-500" },
  }[profil?.role?.toLowerCase()] || { label: "Admin", color: "text-amber-600", bg: "bg-amber-50", border: "border-amber-200", dot: "bg-amber-500" };

  return (
    <div className="min-h-screen bg-slate-50 flex">
      {/* Menggunakan Sidebar khusus Admin */}
        ...
        {memuat ? (
          ...
          <div className="max-w-7xl mx-auto flex flex-col gap-9 mt-3">
            <div className="bg-white rounded-3xl p-6 border border-slate-100 border-b-4 border-b-blue-600 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h1 className="text-xl font-bold text-slate-950">Manajemen Profil Kontrol</h1>
                <p className="text-xs text-slate-400 mt-0.5">Kelola data informasi personal Anda serta pantau ringkasan statistik laporan masuk global</p>
              </div>
              <div className="flex gap-2 w-full sm:w-auto justify-end">
                {editMode ? (
                  <>
                    <button onClick={batal} className="px-4 py-2 text-xs font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors"> Batal </button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
              <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col items-center p-6 text-center">
                ...
                <div className="w-full space-y-2.5 text-left">
                  <div className="bg-slate-50 rounded-xl p-3 flex justify-between items-center">
                    <span className="text-xs font-medium text-slate-400">Terdaftar Sejak</span>
              ...
              <div className="lg:col-span-2 flex flex-col gap-6">
                <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
                  <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Informasi Kredensial Otoritas</h3>
                ...
                <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
                  <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4">Statistik Laporan Platform Global</h3>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-2">
                    ...
                    <div className="bg-slate-50/70 rounded-2xl p-4 border border-slate-200 text-center">
                      <p className="text-2xl font-black text-slate-600">{statistik.menunggu}</p>
                      <p className="text-[10px] font-bold text-slate-400 uppercase mt-0.5 tracking-wider">Menunggu</p>
                    </div>
                    <div className="bg-amber-50/60 rounded-2xl p-4 border border-amber-100 text-center">
                      <p className="text-2xl font-black text-amber-600">{statistik.diproses}</p>
                      <p className="text-[10px] font-bold text-amber-400 uppercase mt-0.5 tracking-wider">Diproses</p>
                    </div>
